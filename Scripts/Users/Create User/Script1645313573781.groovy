...
response = WS.sendRequest(findTestObject('Users/Get User By Email', [('email') : email]))

if (WS.verifyResponseStatusCode(response, 200, FailureHandling.OPTIONAL)) {
	WS.verifyElementPropertyValue(response, '[0].id', GlobalVariable.userId)
	WS.verifyElementPropertyValue(response, '[0].name', name)
	WS.verifyElementPropertyValue(response, '[0].email', email)
	WS.verifyElementPropertyValue(response, '[0].status', status)
	WS.verifyElementPropertyValue(response, '[0].gender', gender)
	
} else if(WS.verifyResponseStatusCode(response, 404)){
	System.out.println("Resource not found");
}

response = WS.sendRequest(findTestObject('Users/Delete User By Id'))
